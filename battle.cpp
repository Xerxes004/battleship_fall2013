...
#include "memory_functions_Kelly.h"                                     // NEW
...
int main() {
   ...
   bool           done = false;
   string         message, dumbComputerMove, smartComputerMove;
   ...
   initializeBoard(smartComputerBoard);
	initMemoryKelly (smartComputerMemory);                              // NEW
   ...
   while (!done) {
      ...
      smartComputerMove = smartMoveKelly(smartComputerMemory);       // CHANGED
      ...
      smartComputerResult = playMove(smartComputerRow, smartComputerColumn,
                                     dumbComputerBoard);
		updateMemoryKelly(smartComputerRow,    smartComputerColumn,        // NEW
      ...
      if (isItSunk(dumbComputerResult))  { numDumbComputerShipsSunk++; }
      if (isItSunk(smartComputerResult)) { numSmartComputerShipsSunk++; }
