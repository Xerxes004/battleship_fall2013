...
//           5 Nov 13 to change class definitions to structs
...
void    welcome       (void);
void    clearTheLine  (int lineNum);
void    clearTheScreen(void);
void    pauseForEnter (void);
string  getResponse   (int lineNum, int colNum, string prompt);
void    writeMessage  (int lineNum, int colNum, string message);
void    writeResult   (int lineNum, int colNum, int    result, int playerType);
void    displayBoard  (int lineNum, int colNum, int    playerType, 
